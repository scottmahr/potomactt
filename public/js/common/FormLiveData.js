...
app.factory('FormLiveData', ['$rootScope','$location','FormGlobals','FormState' ,'FormLifts',
                function($rootScope, $location, FormGlobals, FormState, FormLifts) {
    var fs = FormState;
...
*/
    ...
    service.elevData = function(data,update){
        ...
        if(fs.elevCount%150==0){

            //console.log('elevation',data[0])
        }
        ...
        var accelAvg = FormGlobals.myAverage(fs.accelRaw.wSlice(fs.accelCount-200,fs.accelCount)).mean

        //if(fs.elevCount%25==0){console.log(fs.velocity);}

        if(fs.dataCollectionState=='pause' &&  _.has(fs.lifter,'_id') && $location.path()=='/home' && fs.elevCount>100){
            if(fs.liftType.id == 91){
                service.autoCapture(accel, height,fs.velocity);
            }else if(fs.LDO != undefined ){
                //normal capture
        ...
        if(fs.collectHistData && fs.elevCount%15==0){
            var idx = _.findIndex(fs.bins, function(d) {
                return height-fs.histBaseline < d;
            })
            if(idx==-1){idx=fs.bins.legnth-1;}
            fs.histData[idx]++;

            //console.log(height-fs.histBaseline)
            //console.log(idx)
            //console.log(fs.histData)
            //console.log(fs.bins)
            //console.log('---')


            //this is for the accel now
            idx = _.findIndex(fs.binsAccel, function(d) {
                return accelAvg-fs.histBaselineAccel < d;
            })
            if(idx==-1){idx=fs.binsAccel.legnth-1;}
            fs.histDataAccel[idx]++;

            $rootScope.$broadcast('redarwHistogram',{});
        }
    ...
    service.accelData = function(data){
        ...
        fs.accelRaw[fs.accelCount%1600] = ((data[0]+256*data[1])/8192-1)*9.807;   //change to 0 for gravity
        //maybe capture it
        if(fs.manualCapturing || fs.liftProgress>1){
    ...
    service.elevationCorrection = function(){
        ...
        if(aElevs.variance<.2 && aAccels.variance<.01 ){
            if(aElevs.mean<-.25){
                fs.elevOffset += .25*aElevs.mean
            }else{       //  for now, we are going to adjust everything  if(Math.abs(aElevs.mean)<.5){
